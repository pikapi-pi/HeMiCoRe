...
def mask_percent(np_img):
    if (len(np_img.shape) == 3) and (np_img.shape[2] == 3): #if np_img has 3 channels
        np_sum = np_img[:, :, 0] + np_img[:, :, 1] + np_img[:, :, 2]
        mask_percentage = 100 - np.count_nonzero(np_sum) / np_sum.size * 100  # (np_img.shape[0]*np_img.shape[1])
    else:  #if np_img has only one channel
        mask_percentage = 100 - np.count_nonzero(np_img) / np_img.size * 100  # background's percentage
...
def filter_green_channel(np_img, green_thresh=200, avoid_overmask=True, overmask_thresh=90, output_type="bool"):
    g = np_img[:, :, 1]
    gr_ch_mask = (g < green_thresh) & (g > 0)  #get mask with pixel value in (0, green_thresh)
    mask_percentage = mask_percent(gr_ch_mask)
    # overmask means the background area, so the proportion of it in a tile must be lower than 90%, or
    # by increasing green_thresh until 255 step by step, then decide whether to eliminate the tile
