...
def tiling(slide_filepath, magnification, patch_size, scale_factor=32, tissue_thresh=0.35, method='rgb',
           overview_level=-1, tumour_mask_dir=None, coord_dir=None, overview_dir=None, mask_dir=None, patch_dir=None, filename=None):
    """
    ...
    :param scale_factor: scale wsi to down-sampled image for judging tissue percent of each patch, mainly affact the size of mask
    ...
    :param tumour_mask_dir: the directory of tumour mask
    ...
    # tumour_mask_filepath = str(Path(tumour_mask_dir) / f'{filename}.png')
    # tumour_mask = cv2.imread(tumour_mask_filepath, 0)
    # compute the number of step along x (column) and y (row)
    mask_w, mask_h = mask.size
    # tumour_mask = cv2.resize(tumour_mask, (mask_w, mask_h))
    # tumour_mask = cv2.cvtColor(tumour_mask, cv2.COLOR_GRAY2BGR)
    ...
    # firstly, get the computable area(mask) by various filter, then extract tiles from the mask
        ...
        for col in range(num_step_x):
            ...
            if keep_patch(patch_mask, tissue_thresh, color_bg):  # decide keep or drop the patch by `tissue_thresh`
                ...
                if overview_dir is not None:
                    ...
                    cv2.rectangle(thumbnail, points_thumbnail[0], points_thumbnail[1], color=(0, 0, 255), thickness=5)
                # save patch image.
...
def run(args):
    ...
    for slide_idx, slide_filepath in enumerate(slide_filepath_list):
        ...
        # if filename[:4] != "2015" or len(filename) < 12:
        #     continue
        try:
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='', help="a dir containing all data for exp")
    parser.add_argument('--tumour_mask_dir', type=str, default='/media/lzs/41e45ae4-ad45-41db-9bc2-b8299dd12de8/survival_prediction/data_mask')
    ...
    parser.add_argument('--wsi_format', type=str, default='.svs', choices=['.svs', '.tiff'])
