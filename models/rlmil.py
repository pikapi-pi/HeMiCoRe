...
class ActorCritic(nn.Module):
    ...
    def act(self, state_ini, memory, restart_batch=False, training=False):
        ...
        state, hidden_output = self.gru(state.view(1, state.size(0), state.size(1)), memory.hidden[-1])#state与hidden_output相同
    ...
    def forward(self, x, restart=False):
        if self.fc_rnn:
            if restart:
                ...
                output, h_n = self.rnn(x.view(1, x.size(0), x.size(1)),    #将二维数据变为三维
